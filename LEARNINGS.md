...
**Table Stories:** Title, Text, Current_User, State (whether the story is open and able to be edited or closed)
**Table IP:** ID, Title, User

Grammar Bot API - check the permissibility of the text the user enters to make sure it is comprehensible English.
Scheduler API - to keep track of how long a user is taking to enter a new sentence. Our data is mostly handled as JSON data.
...
6. def get_users(title): Gets list of users writing the specific story. 
7. def leave_story(title): Allows user to leave a story, deletes user from table "IP" 
8. def check_grammar_bot(text): Checks grammar

**Future Work:**
We would like to consider implementing a front end interface for this. Although we started to, we realized we didn't really know yet how to do so.